...
client.on('messageDelete', message =>{ // ивент, когда удаляется любое сообщение с сервера https://discord.js.org/#/docs/main/stable/class/Client?scrollTo=e-guildMemberAdd
  ...
  client.channels.cache.get("815836748981862470").send(embed); // айди вашего канала с логами
})
...
client.on('guildMemberAdd', member =>{ // ивент, когда пользователь присоединяется к серверу https://discord.js.org/#/docs/main/stable/class/Client?scrollTo=e-guildMemberAdd
    ...
    .setDescription(`**мама ебал!
    Ты наш \`${client.guilds.get("811545924856971274").memberCount}\` участник! **`) // айди вашего сервера               !!!!!!!!!!
    .setFooter('иди нахуй :3', 'https://cdn.discordapp.com/emojis/590614597610766336.gif?v=1')
    ...
    .setColor('RANDOM')
    member.send(embed);
    client.channels.cache.get('815836748981862470').send(embed2) // айди вашего канала с логами
})
...
async function change() {
    let members = client.guilds.cache.get("905153679366193162").memberCount // сколько людей на сервере + указать айди своего сервера
    client.channels.cache.get("905153679366193162").setName(`На сервере: ${members}`); // свой айди войса
}
